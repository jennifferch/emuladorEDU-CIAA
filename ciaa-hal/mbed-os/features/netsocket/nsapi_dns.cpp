...
nsapi_error_t nsapi_dns_call_in(call_in_callback_cb_t cb, int delay, sapi::Callback<void()> func)
{
    if (*dns_call_in.get()) {
                                                      ...
                                                      call_in_callback_cb_t call_in_cb, nsapi_version_t version)
{
    ...
    if (!dns_timer_running) {
        if (nsapi_dns_call_in(query->call_in_cb, DNS_TIMER_TIMEOUT, sapi::callback(nsapi_dns_query_async_timeout)) != NSAPI_ERROR_OK) {
            delete query->host;
            delete query;
            dns_mutex->unlock();
            return NSAPI_ERROR_NO_MEMORY;
...
static void nsapi_dns_query_async_initiate_next(void)
{
    ...
    if (query) {
        ...
        nsapi_dns_call_in(query->call_in_cb, 0, sapi::callback(nsapi_dns_query_async_create, reinterpret_cast<void *>(query->unique_id)));
    }
}
...
static void nsapi_dns_query_async_timeout(void)
{
        ...
        if (dns_query_queue[i]) {
            ...
            if (dns_query_queue[i]->total_timeout > DNS_TIMER_TIMEOUT) {
                ...
                if (dns_query_queue[i]->status == NSAPI_ERROR_TIMEOUT) {
                    ...
                    nsapi_dns_call_in(dns_query_queue[i]->call_in_cb, 0, sapi::callback(nsapi_dns_query_async_response, reinterpret_cast<void *>(dns_query_queue[i]->unique_id)));
                }
            }
            ...
            if (dns_query_queue[i]->socket_timeout > 0) {
                if (dns_query_queue[i]->socket_timeout > DNS_TIMER_TIMEOUT) {
                    ...
                    nsapi_dns_call_in(dns_query_queue[i]->call_in_cb, 0,
                                      sapi::callback(nsapi_dns_query_async_send, reinterpret_cast<void *>(dns_query_queue[i]->unique_id)));
                }
            }
    ...
    // Starts timer again
    if (query) {
        nsapi_dns_call_in(query->call_in_cb, DNS_TIMER_TIMEOUT, sapi::callback(nsapi_dns_query_async_timeout));
    } else {
...
static void nsapi_dns_query_async_create(void *ptr)
{
    ...
    if (query->socket) {
        ...
        socket->sigio(sapi::callback(nsapi_dns_query_async_socket_callback, query->socket_cb_data));
...
static void nsapi_dns_query_async_socket_callback(void *ptr)
{
    ...
    if (cb_data) {
        nsapi_dns_call_in(cb_data->call_in_cb, 0, sapi::callback(nsapi_dns_query_async_socket_callback_handle, cb_data->stack));
    }
}
    ...
    if (socket) {
        ...
        while (true) {
            ...
            if (resp < 0) {
                ...
                query->socket_timeout = 0;
                nsapi_dns_call_in(query->call_in_cb, 0, sapi::callback(nsapi_dns_query_async_response, reinterpret_cast<void *>(query->unique_id)));
            }
        }
