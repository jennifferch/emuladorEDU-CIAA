window.MbedJSHal.gpio = (function() {
    ...
    function read(pin) {
        ...
        console.log('read read llegooooooooooooo al read', declaredPins[pin].value);
    ...
    function write(pin, value) {
        ...
        console.log('write write llegooooooooooooo al write', pin);
        if (!(pin in declaredPins)) return;

        if (declaredPins[pin].type === TYPE.DIGITAL) {
            console.log('write write lTYPE.DIGITAL write', pin);

            if (value === 0 || value === 1) {
                declaredPins[pin].value = value;
                console.log('write write declaredPins[pin].value ', declaredPins[pin].value );
        ...
        if (irqPins[pin]) {
            if (value === 0 && irqPins[pin].fall) {
                console.log('write write llegooooooooooooo irqPins[pin].fall');
                ...
                console.log('write write llegooooooooooooo  irqPins[pin].rise');
